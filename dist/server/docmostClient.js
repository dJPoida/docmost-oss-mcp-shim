...
async function login(force = false) {
    const now = Date.now();
    if (!force && now - lastLoginAt < 60_000)
    ...
    try {
        return await pRetry(loginAttempt, {
            ...
            onFailedAttempt: (error) => {
                log(`Login attempt ${error.attemptNumber} failed: ${error.message}. ${error.retriesLeft} retries left.`);
            },
        });
    }
    catch (error) {
...
async function postApi(path, body = {}) {
    ...
    try {
        return await pRetry(apiCall, {
            ...
            factor: retryConfig.factor,
            onFailedAttempt: (error) => {
                log(`API call attempt ${error.attemptNumber} failed: ${error.message}. ${error.retriesLeft} retries left.`);
            },
        });
    }
    catch (error) {
...
export async function searchDocs({ query, spaceId, page = 1, limit = 20, }) {
    // Check cache first
...
export async function getSpacePages(spaceId) {
...
export async function getAttachment(attachmentId, fileName = 'diagram.drawio.svg') {
...
export async function getPageHistory(pageId, page = 1, limit = 20) {
    return postApi('/api/pages/history', { pageId, page, limit });
}
export async function getPageBreadcrumbs(pageId) {
    return postApi('/api/pages/breadcrumbs', { pageId });
}
export async function getComments(pageId, page = 1, limit = 20) {
    ...
    for (const space of spaces) {
        try {
            ...
            const spacePages = Array.isArray(spacePagesResponse)
                ? spacePagesResponse
                : spacePagesResponse.data?.items || [];
            // Add space metadata to each page
            const pagesWithSpace = spacePages.map((page) => ({
                ...page,
                spaceName: space.name,
                spaceSlug: space.slug || space.name.toLowerCase().replace(/\s+/g, '-'),
            }));
...
//# sourceMappingURL=docmostClient.js.map