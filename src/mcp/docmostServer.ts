    ...
    switch (name) {
      ...
      case 'docmost_getAttachment': {
        const { data } = await http.get(
          `/attachments/${args?.attachmentId}${args?.fileName ? `/${args.fileName}` : ''}`
        );
        return {
          content: [
            {
  ...
  try {
    ...
    const resources = [
      ...
      },
      {
        ...
        description:
          'Complete list of all pages with metadata for comprehensive documentation discovery',
        mimeType: 'application/json',
      },
    ];
...
server.setRequestHandler(GetPromptRequestSchema, async (request) => {
  ...
  switch (name) {
    case 'search-docs': {
      const { query, spaceId } = args as { query: string; spaceId?: string };
      return {
